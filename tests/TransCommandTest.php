...
class TransCommandTest extends TestCase
{
    public function testCommandErrorOutputOnMissingKey()
    {
        ...
        $this->artisan('laralang:trans', ['key' => 'users']);
    ...
    public function testCommandErrorOutputOnLanguageNotFound()
    {
        ...
        $this->artisan('laralang:trans', ['key' => 'users.name', '--lang' => 'sd']);
    ...
    public function testCommandAsksForConfirmationToCreateFileIfNotFound()
    {
        ...
        $command = m::mock('\Digicraft\Laralang\Commands\TransCommand[confirm]', [$manager]);
        $command->shouldReceive('confirm')->once()->andReturn(false);

        $this->app['artisan']->add($command);
        $this->artisan('laralang:trans', ['key' => 'users.name']);
    }
    ...
    public function testCommandAsksForConfirmationToCreatePackageFileIfNotFound()
    {
        ...
        $this->app['artisan']->add($command);
        $this->artisan('laralang:trans', ['key' => 'package::file.name']);

        $this->assertFileExists($this->app['config']['laralang.path'].'/vendor/package/en/file.php');
    }
    ...
    public function testCommandExitsWhenFileNotFoundAndConfirmationFalse()
    {
        ...
        $this->assertFileNotExists($this->app['config']['laralang.path'].'/en/users.php');
    }
    ...
    public function testCommandCreatesFileIfNotFoundWhenConfirmed()
    {
        ...
        $this->assertFileExists($this->app['config']['laralang.path'].'/en/users.php');
    }
    ...
    public function testCommandAsksForValuePerLanguageAndWriteToFile()
    {
        ...
        $manager = $this->app[Manager::class];
        $command = m::mock('\Digicraft\Laralang\Commands\TransCommand[ask]', [$manager]);
        ...
        $enFile = (array) include $this->app['config']['laralang.path'].'/en/users.php';
        $nlFile = (array) include $this->app['config']['laralang.path'].'/nl/users.php';
    ...
    public function testCommandAsksForValuePerLanguageForPackageAndWriteToFile()
    {
        ...
        $this->app['artisan']->add($command);
        $this->artisan('laralang:trans', ['key' => 'package::users.name']);

        $enFile = (array) include $this->app['config']['laralang.path'].'/vendor/package/en/users.php';
        $nlFile = (array) include $this->app['config']['laralang.path'].'/vendor/package/sp/users.php';
        $this->assertEquals('name', $enFile['name']);
        $this->assertEquals('naam', $nlFile['name']);
    }
    ...
    public function testCommandAsksForValueForOnlyProvidedLanguage()
    {
        ...
        $this->app['artisan']->add($command);
        $this->artisan('laralang:trans', ['key' => 'users.name', '--lang' => 'en']);
    ...
    public function testCommandAsksForValuePerLanguageForNestedKeysAndWriteFile()
    {
        ...
        $this->app['artisan']->add($command);
        $this->artisan('laralang:trans', ['key' => 'users.name.first']);
    ...
    public function testCommandAsksForLanguageForNestedKeysAndWriteFile()
    {
        ...
        $this->app['artisan']->add($command);
        $this->artisan('laralang:trans', ['key' => 'users.name.first', '--lang' => 'en']);
