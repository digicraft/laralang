...
class RemoveCommandTest extends TestCase
{
    public function testCommandOutput()
    {
        ...
        $command = m::mock('\Digicraft\Laralang\Commands\RemoveCommand[confirm]', [$manager]);
        $command->shouldReceive('confirm')->once()->with('Are you sure you want to remove "user.name"?')->andReturn(true);

        $this->app['artisan']->add($command);
        $this->artisan('laralang:remove', ['key' => 'user.name']);

        $userENFile = (array) include $this->app['config']['laralang.path'].'/en/user.php';
        $userNLFile = (array) include $this->app['config']['laralang.path'].'/nl/user.php';
    ...
    public function testRemovesNestedKeys()
    {
        ...
        $this->app['artisan']->add($command);
        $this->artisan('laralang:remove', ['key' => 'user.name.f']);
    ...
    public function testCommandOutputForVendorPackage()
    {
        ...
        $this->app['artisan']->add($command);
        $this->artisan('laralang:remove', ['key' => 'package::file.not_found']);

        $ENFile = (array) include $this->app['config']['laralang.path'].'/vendor/package/en/file.php';
        $SPFile = (array) include $this->app['config']['laralang.path'].'/vendor/package/sp/file.php';
